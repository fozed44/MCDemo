...
namespace MC {
	...
	public: /* API */
		...
		ID3D12RootSignature* GetRootSignature(const HRootSignature& hSig) const noexcept { return UnwrapHandle(hSig); }

		/*	Use GetRootSignatureHandle to get a signature handler to a standard (built in) signature. */
		HRootSignature GetRootSignatureHandle(STANDARD_ROOT_SIGNATURE standardSignature);
