...
namespace MC {
	...
	typedef MCManagedHandle<ID3DBlob*, ComPtr<ID3DBlob>, MCShaderManager> MCShaderHandle;
	...
	protected:
		void ReferenceCountToZero(ManagedContextType *pContext) override;
	...
	private:
		...
		//std::map<ComPtr<ID3DBlob>, MCShaderContext> _shaders;
